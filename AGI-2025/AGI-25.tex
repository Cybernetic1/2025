...
\node[] (name) at (1.5,2) {\textbf{(RL)}};

\node[] (eye) at (-2, 0.7) {\includegraphics{eye-symbol.png}};
\node[] (mouth) at (-2, 0.2) {\includegraphics{mouth-symbol.png}};
\node[draw,thick,rectangle] (state) at (0, 0.5) {state $x$};
\node[draw,thick,circle,fill=gray!20] (RL) at (3.3, 0.5) {RL};
\node[draw,thick,cylinder,shape aspect=0.2,shape border rotate=90] (LTM) at (2.15,0.4) {LTM};
...
\node[draw, double arrow, double arrow head extend=1.5pt, fill=gray!20, minimum height=26pt, inner ysep=1.6pt] at (1.15,0.5) {};
\end{tikzpicture}
\qquad
...
\node[rectangle, align=left, text depth=5pt, anchor=west, text width=100pt] at (-1, 2) {\textbf{(Type L)} \\ LLM as language model};

\node[draw,thick,circle,fill=gray!20] (RL) at (1.5, 0) {RL};
\node (state) [draw,rectangle split,rectangle split parts=2,minimum width=3em] at (0, 0) {$x_{t+1}$ \nodepart{two} $x_t$};

\node[] (speech1) at (-2,2) {\includegraphics[scale=0.5]{speech-gray.png}};
\node[] (speech2) at (-2,-2) {\includegraphics[scale=0.5]{speech-symbol.png}};

\node (compress1) [draw,trapezium,trapezium angle=-70, fill=gray!20, minimum height=32pt, inner xsep=-2pt] at (-2,0.55) {NL};
\node (compress2) [draw,trapezium,trapezium angle=70, fill=gray!20, minimum height=32pt, inner xsep=-6.5pt, below=0pt of compress1] {NL${}^{-1}$};
...
\node[] (world1) at (-2,1.9) {\includegraphics[scale=0.5]{world-gray.png}};
\node[] (world2) at (-2,-1.9) {\includegraphics[scale=0.5]{world-symbol.png}};

\node (compress1) [draw,trapezium,trapezium angle=-70, fill=gray!20, minimum height=40pt, inner xsep=2pt] at (-2,0.67) {};
\node (compress2) [draw,trapezium,trapezium angle=70, fill=gray!20, minimum height=40pt, inner xsep=2pt, below=of compress1] {};

\node (stateA) [draw,trapezium,trapezium angle=-70, fill=white, minimum height=10pt, inner xsep=11.5pt, above=-25pt of compress1] {?};
\node (stateB) [draw,trapezium,trapezium angle=70, fill=white, minimum height=10pt, inner xsep=11.5pt, below=-25pt of compress2] {?};
...
\draw[postaction={decorate},rounded corners=12pt] (RL.south) to ([yshift=-15pt]RL.south) to ([yshift=-14pt]state.south) to (state.south);
\end{tikzpicture}
\caption{Two types of RL + LLM architectures.}
\end{figure}
...
\begin{figure}
	...
	\caption{String diagram of the Logic Transformer.  The {\LARGE\color{magenta}\RIGHTarrow} indicates \textbf{internal data} supplied by the Transformer, similar to the Q,K,V matrices in traditional Transformers.  These are modified through ``learning'' or training.  The 2 \RIGHTarrow's \ are \textbf{inputs} to the Transformer, this is matched with the 2 \textbf{outputs} on the right of the diagram.  Tags {\color{orange}$[M,N,...]$} indicate the \textbf{dimensions} of data streams.}
...
A very interesting question concerning Part II is how to handle variable-length propositions?  If this could be done, the neural network may have a form very close to or even identical to current Transformers.  Would that mean
