...
Next we look at two types of architectures for combining RL and LLM (Other researchers have proposed different taxonomies \cite{?}).  \textbf{Type A} is the ``mainstream'' approach, of which RLHF seems to be one instance.  Here, ... Ever since Richard Montague's success in converting a fragment of English into formal logic \cite{Montague}.  \textit{The ``format'' of human internal thought is natural language.}
\begin{equation}
\hspace{-0.5cm}
\begin{tikzpicture}[node distance=-2pt,every path/.style={ultra thick},every text node part/.style={align=center},decoration={
	markings,mark=at position 0.53 with {\arrow{>}}}]
