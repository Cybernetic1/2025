...
One unconventional feature in my RL theory is that a ``thought'' is considered an ``action.''  After some analysis I find it to be philosophically valid.  This view seems to be shared by mainstream research groups also (eg. RLHF), as when they set up RL with LLM, using the output token as action.
...
The types L and W are not mutually exclusive.  I guess they can both be incorporated into the same RL system, but I have not worked out the details.
