...
现在我们开始分析 状态空间的「形状」，以下是一个 \textbf{状态转移} 的例子：
...
\begin{equation}
	...
	\end{tabular}
\end{equation}
这幅图像很简单清晰，但其实每个状态 $x$ 虽然只是一点，它却有复杂的内部结构，可以看作是这个点的 \textbf{坐标}。 

\centering $\mathbb{P} =$ \oldtextbf{proposition space (命题空间)} \vspace{-1em}
\begin{equation}
...
\centering $\bbOmega =$ \oldtextbf{symbol space (符号空间)} \vspace{-1em}
\begin{equation}
